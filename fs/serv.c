...
serve_write(envid_t envid, struct Fsreq_write *req) {
  ...
  //cprintf("RETURN FROM SERVE_WRITE\n");
	return count;
}
...
int serve_snpsht(envid_t envid, union Fsipc *req)
{
  ...
  while ((c=*(cmd++))!='\0')
  {
    if (c=='-')
    {
      ...
      switch(c)
      {
        ...
        case 'b':
          action =c;
          cprintf("You want bitmap? Okay.\n");
          break;
        case 'h':
          action =c;
          cprintf("Do you need help?\n");
          break;
      }
    }
    else if (c==' ')
  ...
  if ((c == '\0') && (action != 'p') && (action != 'e') && (action!= 'b') && (action!='h'))
    return -1;
  ...
  if (action == 'c')
  {
    ...
    if (type == 'i')
      return create_snapshot(cmd+i, cmd);
    else if (type == 'f')
      return create_snapshot_full(cmd+i,cmd);
  }
  ...
  if (action == 'a')
  {
    ...
    if (type == 'i')
      ...
      return accept_full_snapshot(cmd);
  }
  ...
  if (action == 'd')
  {
    ...
    if (type == 'i')
      ...
      return delete_full_snapshot(cmd);
  }
